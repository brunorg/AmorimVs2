...
function CarregarRotina() {
    ...
    $('#Rotina_Semanal_Dia').html(diasSemana[diaHoje]);
        ...
        success: function(result) {
            ...
            estruturarTabelaRotinaDia(result);
...
function rotinaMudarDia(quanto) {
    var diaTutoria = parseInt($("#diaTutoriaAluno").val());
...
function verificarCicloPeridoAluno() {
    ...
    var ciclo;
    var periodo = parseInt(alunoVariavel.periodo.idperiodo);

    // Define o ciclo pelo ano de estudo do aluno
...
function estruturarDadosRotina(dados, rotina) {
    var horarios = [];
    var rodaConversa = 0;
    var recreio = 0;
    var tutoria = 0;

    // Define o dia da semana no qual o aluno terá tutoria
    if (dados.ciclo === 1 || dados.ciclo === 2)
        tutoria = 2;
    else if (dados.ciclo === 3)
        tutoria = 4;
    else
        return false;

    // Define os horários disponíveis para rotina de acordo com o ciclo e período do aluno
    if (dados.periodo === 8) {
        if (dados.ciclo === 1 || dados.ciclo === 2) {
            recreio = 10;
            rodaConversa = 10.5;
            horarios = [7, 8, 9, 10, 10.5, 11];
        } else if (dados.ciclo === 3) {
            recreio = 9.5;
            rodaConversa = 10;
            horarios = [7, 8, 9.5, 10, 11];
        }
    } else if (dados.periodo === 9) {
        if (dados.ciclo === 1 || dados.ciclo === 2) {
            recreio = 15;
            rodaConversa = 15.5;
            horarios = [13, 14, 15, 15.5, 16, 17];
        } else if (dados.ciclo === 3) {
            recreio = 15.5;
            rodaConversa = 16;
            horarios = [13, 14, 15.5, 16, 17];
        }
    } else {
        return false;
    }
    ...
    return {
        horarios: horarios,
        recreio: recreio,
        rodaConversa: rodaConversa,
        tutoria: tutoria
    };
}

function estruturarTabelaRotinaDia(rotina) {
    var dados = verificarCicloPeridoAluno();
    var estruturaDados = estruturarDadosRotina(dados, rotina);
    var htmlTable = "";

    // Estruturar a tabela de visualização da rotina do alunos
    for (var i = 0; i < estruturaDados.horarios.length; i++) {
        if (estruturaDados.horarios[i] === estruturaDados.recreio) {
            var hrRecreio = estruturaDados.recreio % 1 === 0 ? 
                                estruturaDados.recreio + "h00" : 
                                estruturaDados.recreio.toString().split(".")[0] + "h30";

            htmlTable += "<tr id=\"Rotina_Semanal_Linha_Recreio\" class=\"Rotina_Semanal_Linha Rotina_Semanal_Linha_Recreio\">";
            htmlTable +=     "<td class=\"Rotina_Semanal_Horario\">"+hrRecreio+"</td>";
            htmlTable +=     "<td class=\"Rotina_Semanal_Recreio\" colspan=\"3\">RECREIO</td>";
            htmlTable += "</tr>";
        } else if (estruturaDados.horarios[i] === estruturaDados.rodaConversa) {
            var hrConversa = estruturaDados.rodaConversa % 1 === 0 ? 
                                estruturaDados.rodaConversa + "h00" : 
                                estruturaDados.rodaConversa.toString().split(".")[0] + "h30";

            htmlTable += "<tr id=\"Rotina_Semanal_Linha_RodaConversa\" class=\"Rotina_Semanal_Linha Rotina_Semanal_Linha_Recreio\">";
            htmlTable +=     "<td class=\"Rotina_Semanal_Horario\">"+hrConversa+"</td>";
            htmlTable +=     "<td class=\"Rotina_Semanal_Recreio\" colspan=\"3\">RODA DE CONVERSA</td>";
            htmlTable += "</tr>";
        } else {
            var hrOficina = estruturaDados.horarios[i] < 10 ? "0" + estruturaDados.horarios[i] : estruturaDados.horarios[i];
    ...
    $("#diaTutoriaAluno").val(estruturaDados.tutoria);
    $("#Rotina_Semanal_Tabela").find("tbody").html(htmlTable);
}
...
function carregarTabelaTutoria() {
    ...
    $("#Rotina_Semanal_Tabela").find("tbody")
        .find("tr")
        .not("#Rotina_Semanal_Linha_Recreio")
        .not("#Rotina_Semanal_Linha_RodaConversa")
        .each(function(a) {
            $(this).find(".Rotina_Semanal_Materia").html("Tutoria");
            $(this).find(".Rotina_Semanal_Professor").html(tutoria.tutoria.split(" ")[0]);

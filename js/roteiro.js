...
function verificaProducaoExistente(Numero, IDRoteiroLocal)
{
	...
	if(Numero==1)
	{
		...
		if(!Encontrado){
			return '<a style="text-align:right;color:white" onclick="showUpload(2,'+IDRoteiroLocal+');" href="#">Fichas de Finalização</a>';			
		}
	}
...
function LoadRoteiro(){
	...
	var HtmlContent;
        ...
        Limite = dataRoteiro.length;
       	HtmlContent = "";

        for(var a = 0; a < Limite; a++)
        {
       		if(usuario == "Professor" || (dataRoteiro[a].anoEstudo.idanoEstudo == AnoEstudoID && usuario == "Aluno"))
       		{
...
function SalvarPortifolio()
{
    ...
    Arquivo != undefined)
  {
      ...
      crossDomain: true,
      dataType: 'json',
      //data: "action=create&anoLetivo=8&texto="+$('#Roteiro_Id_'+roteiroAcionado+' .roteiro_nome_tabela_texto').html()+"&data="+dataSalvaPortifolio.getUTCDate()+"-"+(dataSalvaPortifolio.getUTCMonth()+1)+"-"+dataSalvaPortifolio.getUTCFullYear()+"&aluno="+getAlunoByUsuario(usuarioId)+"&tipo="+PortifolioVariavel+"&categoria=3&roteiro="+roteiroAcionado,

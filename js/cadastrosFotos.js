	...
	for(var a=0;a<dataAlunoVariavel.length; a++){	
		if((dataAlunoVariavel[a].anoEstudo.idanoEstudo == anoEstudo ||(anoEstudo == ""))&&(dataAlunoVariavel[a].periodo.idperiodo == periodo || periodo == "")){			
			...
			if(dataAlunoVariavel[a].aluno.fotoAluno != "http://177.55.99.90/files/Masc.png" && dataAlunoVariavel[a].aluno.fotoAluno != "http://177.55.99.90/files/Fem.png" && dataAlunoVariavel[a].aluno.fotoAluno != null ){
				...
				HtmlContent += '<tr id="aluno'+dataAlunoVariavel[a].aluno.idAluno+'" '+temFoto+' onClick="abreBoxSalvarFoto('+dataAlunoVariavel[a].aluno.idAluno+',\'professor\')">'+
...
function carregaProfessor(todos){
	...
	{	
		if(todos == "todos" && dataProfessorFV[d].professorFuncionario != null){
			HtmlContentP += '<tr id="aluno" onClick="abreBoxSalvarFoto('+dataProfessorFV[d].professorFuncionario.idprofessorFuncionario+');" >'+
			...
			if(dataProfessorFV[d].periodo !=null){
				if(dataProfessorFV[d].periodo.idperiodo == periodoP && dataProfessorFV[d].professorFuncionario!=null){
					HtmlContentP += '<tr id="aluno" onClick="abreBoxSalvarFoto('+dataProfessorFV[d].professorFuncionario.idprofessorFuncionario+');">'+
									'<td class="alunoNome">'+dataProfessorFV[d].professorFuncionario.nome+'</td>'+
...
function abreBoxSalvarFoto(id){
	$(".blackPainel").css("display","block");
	$("#professorFuncionario").val(id);
}

...
function LoadRoteiro(){
	...
	var HtmlContent;
        ...
        {
       		if( dataRoteiro[a].anoEstudo.idanoEstudo == AnoEstudoID && usuario == "Aluno")
       		{
...
function SalvarPortifolio(){
	...
	for(var i=0;i<dataAnoLetivo.length;i++){
		...
		if(anoLetivo[0]==dataSalvaPortifolio.getUTCFullYear()){
			var anoAtual = dataAnoLetivo[i].idanoLetivo;
		}
	}
  	...
  	if(Arquivo != "" && Arquivo != undefined) {
		$.ajax({
			...
			type: "POST",
			crossDomain: true,
			data: "action=create&anoLetivo="+anoAtual+"&texto="+$('#Roteiro_Id_'+roteiroAcionado+' .roteiro_nome_tabela_texto').html()+"&data="+dataSalvaPortifolio.getUTCFullYear()+"-"+(dataSalvaPortifolio.getUTCMonth()+1)+"-"+dataSalvaPortifolio.getUTCDate()+"&aluno="+getAlunoByUsuario(usuarioId)+"&tipo="+PortifolioVariavel+"&categoria=1&roteiro="+roteiroAcionado,
			success: function(d) {
				addFileTo(d);
				$('.blackPainel').hide();

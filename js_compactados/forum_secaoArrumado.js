...
$(document).ready(function(){
		...
		if($(".Pergunta_Input").val().length >= 10)
		{
	    	...
	    	for(var a=0;a<dataAluno.length; a++)
	    	{
	    						...
	    						// '<div class="Box_pergunta_wrap"> '+
	    							...
	    							// '<div class="box_pergunta"> '+
	    								// '<div class="pergunta_header"> '+
	    									...
	    									// (IdAluno == dataAluno[a].idAluno ? '<div id="bt_fechar" onclick="deletarMensagemQ('+style="float:right"></div>':'')+
	    								// '</div> '+
	    								// '<div class="Pergunta"> '+
	    								// '</div> '+
	    							// '</div> '+
					...
					$("#frmCadastro_Questao").unbind('submit').on("submit", function(event) {
	                    ...
	                    data: formData,
	            			...
	            			success: function(d) {
	            				...
	            				$("#FORUM .Pergunta_Resposta_Container").last().attr("idQ",""+d);
	    						...
	    						'<div class="Box_pergunta_wrap"> '+
	    							...
	    							'<div class="box_pergunta"> '+
	    								'<div class="pergunta_header"> '+
	    									'<div class="nome_serie_aluno"> '+dataAluno[a].nome+" | "+getSerieAluno()+"º Série - "+getPeriodoAluno()+' </div> '+
	    									(IdAluno == dataAluno[a].idAluno ? '<div id="bt_fechar" onclick="deletarMensagemQ('+ d + ')" style="float:right"></div>':'')+
	    								'</div> '+
	    								'<div class="Pergunta"> '+
	    								'</div> '+
	    							'</div> '+
	    		...
	    		} else if((dataProfessorFuncionario[a] != undefined ? (dataProfessorFuncionario[a].idprofessorFuncionario == IdAluno && usuario == "Professor"):false))
	    		{
	    			...
	    			boxHtml = ' '+
	    						'<div class="Pergunta_Resposta_Container" idQ="0"> '+
	    						'<div class="Box_pergunta_wrap"> '+
	...
	/*$('.perguntar_anexo').click(function(){

		$(this).parent().parent().find('#anexo').change(function(e){

			$(this).parent().find('.perguntar_anexo_nome').html($(this).val());
	...
	$(document).on('click', '.bt_responder', function() {
		...
		$(".selecionado .mostrar_respostas").removeClass("mostrar_respostas").addClass("ocultar_respostas").html("ocultar respostas");
			...
			/*$('.responder_anexo').click(function(){

				$(this).parent().parent().find('#anexo').change(function(e){

					$(this).parent().find('.responder_anexo_nome').html($(this).val());

...
function flood_fill(image_data, canvas_width, canvas_height, x, y, _color) {
    ...
    var startA = image_data[pixel_pos + 3];
    
    if(fillColorR==startR && fillColorG==startG && fillColorB==startB)
        return;  //prevent inf loop.

    function matchStartColor(pixel_pos) {
      return (startR - 30 < image_data[pixel_pos] && image_data[pixel_pos]  < startR + 30) && 
             (startG - 30 < image_data[pixel_pos+1] && image_data[pixel_pos+1] < startG + 30) &&
             (startB - 30 < image_data[pixel_pos+2] && image_data[pixel_pos+2] < startB + 30) &&
             (startA - 30 < image_data[pixel_pos+3] && image_data[pixel_pos+3] < startA + 30);
    }
